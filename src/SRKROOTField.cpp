...
int SRKROOTField::loadFieldTXTFile(string fieldPath, double scalingValue)
{
    ...
    cout << "startCorner"; startCorner.Print();
    cout << "spacing"; spacing.Print();
    cout << "endCorner"; endCorner.Print();
    cout << "length"; length.Print();
...
//        }
        ...
        for (int i = 0;i< dimOfField;i++)
        {
            ...
            resultVec[i]=theField[i][x][y][z]*a1+
                theField[i][x][y][zp1]*a2+
                theField[i][x][yp1][z]*a3+
                theField[i][x][yp1][zp1]*a4+
                theField[i][xp1][y][z]*a5+
                theField[i][xp1][y][zp1]*a6+
                theField[i][xp1][yp1][z]*a7+
                theField[i][xp1][yp1][zp1]*a8;
...
//        spacing.Print();
        ...
        for (int i = 0;i< dimOfField;i++)
        {
            ...
            resultVec[i]=theField[i][x][y][0]*(modxm)*(modym)+
                theField[i][x][yp1][0]*(modxm)*(mody)+
                theField[i][xp1][y][0]*(modx)*(modym)+
                theField[i][xp1][yp1][0]*(modx)*(mody);
